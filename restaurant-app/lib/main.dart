...
import 'package:restaurant_app/features/auth/screens/splash_screen.dart';
...
void main() async {
  ...
  try {
    // الانتظار لثانية واحدة كحد أقصى لجلب لون الثيم لضمان عدم تأخر إقلاع التطبيق
    final settings = await apiService.getPublicSettings().timeout(const Duration(seconds: 1));
    if (settings['restaurantThemeColor'] != null) {
      final hexColor = settings['restaurantThemeColor'].toString().replaceAll('#', '');
  ...
  runApp(RestaurantApp(initialPrimaryColor: primaryColor));
}

class RestaurantApp extends StatelessWidget {
  final Color initialPrimaryColor;
  const RestaurantApp({super.key, required this.initialPrimaryColor});
